...
| NSP-164 | ~~Ambient Mode — Suggestions inbox, Settings, disclosure~~ — **done 2026-08-22** | Phone | `AmbientSuggestionsInboxView` (accept → real freestanding `Action`, evidence `[]`, same "human directly asserted this" reasoning `ActionComposerView` uses; reject → discarded); `AmbientModeView` (Start/Stop, elapsed time, "N caught so far," the continue/stop reprompt); `AmbientModeDisclosureView` (one-time, hard-to-miss consent screen, required before first use); a Settings → Ambient section (enable toggle + duration picker, 30/60/90/120/150 min). Entry point is My Work's toolbar, next to Weekly Brief. | — |
| NSP-165 | ~~Ambient Mode — "Recording in Process" disclosure cue~~ — **done 2026-08-22** | Phone | `AVSpeechSynthesizer` speaks "Recording in Process" out loud on every session start and every duration-limit renewal — the locked decision (audible, not just visible, and the stronger of the two phrasings considered) from the "Overheard" artifact. | — |
...
| NSP-174 | Ambient Mode Phase 3 — Watch remote control | Watch, NSPTransfer | **Not started — planned, not built.** Watch's only sanctioned path to an ongoing mic session (`AudioRecordingIntent` + its mandated ongoing system presentation) shows the identical system recording indicator a Meeting does — on-watch ambient listening would look and feel exactly like recording a meeting to anyone nearby, the opposite of this feature's premise. Watch's role stays a remote start/stop/continue-prompt toggle for the iPhone-side `AmbientCoordinator` session, the same relationship the Watch already has to meeting recording today (docs/03's transfer/state-mirroring machinery, not a new capture path) — never processing audio on-device on the Watch itself. | — |
