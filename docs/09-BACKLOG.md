...
| NSP-149 | Scheduled Recording | Phone | Pre-scheduled recording (manual or calendar-imported) with a mandatory Start/Skip local notification (per-schedule alert style: sound/vibrate/silent) and unattended auto-stop; Start always enters the normal Arming path, never a shortcut around a workspace-required announcement. `docs/07` § 2.1. One open risk needs a device spike before the default-off `FeatureFlag.scheduledRecording` can be enabled: EventKit full-access behavior alongside the existing write-only calendar grant. (The other open risk this ticket used to name — whether a live recording and its auto-stop timer survive backgrounding — is resolved by `NSP-175`'s `UIBackgroundModes: audio`.) | INT-001 |
...
still starts from an explicit tap. **One risk stays open regardless — a genuine engineering fact, not a policy
choice**: unverified on real hardware/audio (this environment can't test a live microphone) — see `NSP-163`.
The foreground-only risk this note used to name is resolved by `NSP-175`'s `UIBackgroundModes: audio`.
...
| NSP-163 | ~~Ambient Mode — live capture coordinator~~ — **done 2026-08-22, needs on-device verification** | Phone | `AmbientCoordinator` — deliberately not `RecordingSession`/`CaptureEngine`; never writes an audio file, never produces a `Meeting`/`Segment`. Continuous on-device `SFSpeechRecognizer` (`requiresOnDeviceRecognition = true`) via a rotating-request pattern (a stable `RecognitionRequestBox` the audio tap captures once; the underlying recognition task swaps out every ~50s so an hours-long session survives whatever undocumented continuous-session limit `SFSpeechRecognizer` has). Duration timer + "Continue Ambient Mode?" reprompt. Survives backgrounding as of `NSP-175`'s `UIBackgroundModes: audio`. Unverified on real hardware/audio — this environment has no way to speak into a microphone. | — |
| NSP-175 | ~~Background recording + Lock Screen Live Activity~~ — **done 2026-08-22, needs on-device verification** | Phone (`project.yml`, `App/Shared`, `App/Widgets`) | Closes the `UIBackgroundModes: audio` gap `NSP-149`/`NSP-163` both flagged: declared app-wide, so `RecordingSession`/`AmbientCoordinator`'s `AVAudioSession` keeps iOS from suspending the app once the screen locks or it backgrounds, for every capture mode (Meeting, Mental Note, Exercise Mode) — matching Voice Memos, the user's explicit reference point. Also adds the Lock Screen/Dynamic Island Live Activity `docs/03` § 2.2 always specced but never built: a new `NorthStarWidgets` WidgetKit extension target renders `CaptureActivityAttributes` (`App/Shared`, the cross-target contract so neither target imports the other); `CaptureActivityController` (`App/Phone`) starts/updates/ends it as `RecordingSession`/`AmbientCoordinator` state changes — strictly a one-way *view*, per that section's own rule, never a source of truth. Read-only today (tap-to-open, no Stop button on the card itself) — an interactive Stop needs an `AppIntent` plus a way to signal back into the host process (an App Group, most likely), scoped out as follow-up. Unverified on real hardware — this environment can't lock a physical device and observe the Lock Screen. | — |
...
**"The First Hour"** — the skippable, resumable "Getting Started" wizard evaluated and built 2026-08-22 (see
the recommendation artifact for the full walkthrough, question bank, and diagrams). Additive to — never a
replacement for — the two onboarding surfaces that already existed and stay mandatory: `docs/06` § 3.1's
non-skippable legal disclosure and `docs/07` § 12's permission sequence + first-recording success criteria,
including § 12.3's one-time processing-mode choice, which this wizard's own "How I Record" step only confirms,
never re-asks. Gated behind `FeatureFlag.gettingStartedWizard`, default off.

| NSP-176 | ~~Data model + flag~~ — **done 2026-08-22** | NSPCore, NSPPersistence | `Person` gained `role: String?`/`organization: String?` (Migration024) — freeform, never required, first populated for the self-Person by the wizard's identity step but usable on any Person. `Policy` gained `onboardingState: OnboardingState` (`.notStarted/.inProgress/.skipped/.completed`) and `onboardingLastStepIndex: Int` (Migration025) — see `OnboardingState`'s own doc comment for the auto-present/resume rules. `WorkspaceRepository` gained `update` (previously insert/find only) so the wizard can rename the placeholder "My Workspace." `FeatureFlag.gettingStartedWizard` added, default off. | — |
| NSP-177 | ~~Wizard shell + Welcome/Identity/Review~~ — **done 2026-08-22** | Phone | `GettingStartedCoordinator` (thin, same `@MainActor @Observable` shape as `RecordingSession`/`AmbientCoordinator`) — every step commits to its repository the moment it's answered, never a deferred "Save," so `Policy.onboardingLastStepIndex` always reflects real, already-persisted progress and a force-quit mid-wizard loses nothing. `GettingStartedWizardView` is the shell (progress dots, Skip/Back/Continue, the assistant's "speech bubble" framing). Auto-presented once from `RootView`, gated on `OnboardingState.shouldAutoPresent` and the feature flag — strictly after the mandatory legal disclosure/permission sequence, which both happen before `RootView` ever mounts. Reachable again anytime from Settings → Getting Started, label switching between "Get Started"/"Continue Getting Started"/"Redo Getting Started". | — |
| NSP-178 | ~~Threads + People steps~~ — **done 2026-08-22** | Phone | `WizardThreadsStep`/`WizardPeopleStep` — repeatable rows, each committing a real `NSPThread`/`Person` on "Add," not on "Continue." Threads step offers `ThreadKind.allCases` (including `.household`). People step offers a freeform relationship-tag field with common suggestions and multi-select chips to attach the new person to any Thread already created this session (`ThreadParticipantRepository.setParticipants`). Zero entries in either step is a valid answer. | — |
| NSP-179 | ~~Calendar step~~ — **done 2026-08-22** | Phone | `WizardCalendarStep` — explicitly does **not** modify the user's real calendar (`EventKitCalendarEventWriter` is never called from this flow); reads via the existing `CalendarEventReader` (full-access, read-only, the same one `NSP-149`'s calendar-import path uses) and lets the user check which of the next two weeks' events to bring into the app's own tracking layer. Each checked event becomes a real `ScheduledRecording` row — the same entity `CalendarEventPickerView`'s existing single-event import already creates — optionally grouped under a new, user-named `Project`. Declining calendar access, or checking nothing, never blocks continuing. **Depends on the same EventKit full-access device spike `NSP-149`'s own ticket text already flags as open** — this step is not a new instance of that risk, it's the second feature waiting on the same unresolved spike. | — |
| NSP-180 | ~~Recording prefs, Exercise Mode, Sync steps~~ — **done 2026-08-22** | Phone | `WizardRecordingPrefsStep` is read-only — it confirms whatever `docs/07` § 12.3 already had the user choose, never a second prompt. `WizardAmbientStep`/`WizardSyncStep` mirror Settings' own Exercise Mode and iCloud sync toggles exactly, including the 5-minute wheel picker — no new logic, just wizard-framed copy over the same `Policy` fields. | — |
| NSP-181 | ~~Follow-up sessions ("talk to your assistant again")~~ — **done 2026-08-22** | Phone | `FollowUpWizardView` — reachable only once `onboardingState == .completed`, from Settings ("Talk to Your Assistant Again") and My Work's toolbar; both rows carry explanatory copy so the follow-up path is actually discoverable, not just technically present. Deliberately a different flow shape from the first-run wizard: opens on a topic checklist (Threads/People/Meetings/Action Items) and only walks through what's checked, reusing the exact same step views and `GettingStartedCoordinator` write methods — same data, same commits-immediately behavior, different entry sequence. The Action Items topic reuses `ActionComposerView` directly (freestanding, `meetingID: nil`) rather than a bespoke action-editing UI. A follow-up session never touches `Policy.onboardingState`/`onboardingLastStepIndex` — only the first-run wizard's own `advance()`/`finish()`/`skipEntirely()` do. | — |

**Test coverage gap-fill, same session (2026-08-22)** — three systems this backlog already marked done had
thinner test coverage than their own importance implied; closed alongside the wizard above rather than as a
separate pass. "The Spine": `RelationshipGraph` (`NSP-166`) and `WeeklyBriefComposer` (`NSP-169`) had zero
dedicated test files before now — `RelationshipGraphTests.swift`/`WeeklyBriefComposerTests.swift` (NSPIntelligence,
10 tests) cover `tiedToThread`/`tiedToPerson`/`tiedToProject`, action dedup across meeting+counterparty overlap,
quiet-thread/overdue-follow-up/decision-window logic, and the `.closed`-thread exclusion rule. "Overheard":
`AmbientSuggestion`'s repository layer had no test file (only the relevance gate/extractor that produce
suggestions did) — `GRDBAmbientSuggestionRepositoryTests.swift` (5 tests) covers insert/round-trip, the
accept/reject-removes-from-pending-but-keeps-the-row rule, and thread/counterparty match persistence. `NSPThread`
and `Project` also gained their own repository test files (9 tests combined) — previously covered only
indirectly through `DashboardComposerThreadsTests`'s join assertions.
